  ...
  def _log_pruning_metrics(self, logs, step):
    with self._file_writer.as_default():
      for name, value in logs.items():
        tf.summary.scalar("2/" + name, value, step=step)
    ...
    if self.update_freq and epoch % self.update_freq == 0:
      ...
      with self.file_writer.as_default(), tf.summary.record_if(True):
        for i, name in enumerate(self.evaluator.metric_names):
          tf.summary.scalar("AP/" + name, metrics[i], step=epoch)
          eval_results[name] = metrics[i]
      return eval_results


class DisplayCallback(tf.keras.callbacks.Callback):
  """Display inference result callback for one test image."""
...
class DisplayInferenceTestImagesCallback(tf.keras.callbacks.Callback):
  """Display inference result callback for list of test images."""

  def __init__(self, sample_image, output_dir, update_freq=None):
    super().__init__()
    self.list_of_images = dict()
    for i in range(len(sample_image)):
      filename = os.path.basename(sample_image[i])
      image_file = tf.io.read_file(sample_image[i])
      sample_image_tmp = tf.expand_dims(
          tf.image.decode_jpeg(image_file, channels=3), axis=0)
      # self.list_of_images.append(sample_image_tmp)
      self.list_of_images[filename] = sample_image_tmp
    self.update_freq = update_freq
    self.output_dir = output_dir
  ...
  def _draw_inference(self, step):
    ...
    with self.file_writer.as_default():
      for filename, image_ in self.list_of_images.items():
        results = self.model(image_, training=False)
        boxes, scores, classes, valid_len = tf.nest.map_structure(np.array, results)
        length = valid_len[0]
        image = inference.visualize_image(
            image_[0],
            ...
            min_score_thresh=self.min_score_thresh,
            max_boxes_to_draw=self.max_boxes_to_draw)
        tf.summary.image('Test image/' + filename, tf.expand_dims(image, axis=0), step=step)
    self.model.__class__ = EfficientDetNetTrain
...
def get_callbacks(params, val_dataset=None):
  ...
  if params['moving_average_decay']:
    ...
    ckpt_callback = tf.keras.callbacks.ModelCheckpoint(
        os.path.join(params['model_dir'], 'model.ckpt-{epoch:d}'),
  ...
  if params['model_optimizations'] and 'prune' in params['model_optimizations']:
    ...
    tb_callback = tf.keras.callbacks.TensorBoard(
        log_dir=params['model_dir'],
        update_freq=params['num_epochs'],
        histogram_freq=params['num_epochs'],
        write_graph=False,
        profile_batch=2 if params['profile'] else 0)
    callbacks.append(tb_callback)
  if params.get('sample_image', None):
    display_callback = DisplayInferenceTestImagesCallback(
  ...
  # add progress bar step on the same line
  tqdm_callback = tfa.callbacks.TQDMProgressBar(
      leave_overall_progress=True, show_overall_progress=False)
